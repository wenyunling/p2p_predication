...
#define CHANGED_OPTION(optionName, oldValue, newValue)                                                                                               \
    {                                                                                                                                                \
        StrategyOptionsChangedEvent(optionName, lexical_cast<std::string>(oldValue), lexical_cast<std::string>(newValue));                           \
    }
 ...
 *
 * This class provides a BitTorrent client application model for ns-3. The client model ships with an implementation of the BitTorrent protocol that
 * adheres to Bram Cohen's official <a href="http://www.bittorrent.org/beps/bep_0003.html" target="_blank">BitTorrent Protocol Specification</a> and
 * provides client logic according to the descriptions found on the <a href="http://wiki.theory.org/BitTorrentSpecification"
 * target="_blank">BitTorrent Protocol Specification on wiki.theory.org</a>. This class is a centralized point of information storage and -exchange
 * for one instance of the BitTorrent client model.
 *
 * The central idea of the client model is the ability for researchers to exchange or enhance the client logic in order to test protocols based on the
 * BitTorrent paradigm with low effort. For this reason, the client logic is intended to be subdivided into logical units referred to as "strategies".
 * Classes implementing strategies can be notified of BitTorrent's Peer Wire protocol messages and exchange information with other classes of the same
 * client instance by triggering and listening to events handled by this BitTorrentClient class. Communication with other members of a BitTorrent
 * swarm is provided by methods of the Peer class.
 *
 ...
 * To see a very basic example on the encouraged way of implementing strategies, open the implementation of the RequestSchedulingStrategyBase class in
 * the related file. \n \n \n The methods provided by this class can be subdivided into the following logical subsets. The ordering in the list of
 * methods adheres to the order of these subdivisions:
 *
 * <b>1. General methods and settings</b>\n
 * <i>Constructors and related methods</i>: Used for the initialization of the client class, choosing the shared file to download, the employed
 * downloading method (SetTorrent and SetProtocol methods, respectively), the initial download status of the client (bitfield manipulation methods)
 * and starting up the client (DoInitialize method).\n <i>Report generation methods</i>: Methods used to generate periodic output by the client for
 * user-defined metrics.\n <i>Local client information</i>: Grants access to information about the client such as ID in the cloud or uplink/downlink
 * bandwidth information.
 *
 * <b>2. Client behavior settings</b>\n
 * <i>Cloud member behavior methods</i>: Allow to define when the client shall connect to the cloud during its lifetime and the length of its lifetime
 * as a seeder.\n <i>BitTorrent parameters</i>: Control the central parameters of a client within a BitTorrent network, such as number of peers and
 * request sizes.
 *
 * <b>3. Internal data</b>\n
 ...
 * <i>Download/upload-related events</i>: Events triggered by the (partial) download/upload of a certain block of the shared file from/to a remote
 * client.\n <i>Settings change events</i>: Triggered when client or strategy settings are changed (e.g., using the methods in 2. and 3.).\n <i>Report
 * generation events and methods</i>: Triggered to gather information within the client for output (cf. the methods in 1.).
 *
 ...
 * <i>Cloud connection methods</i>: Methods that may be called from outside the client (e.g., a controlling script) to establish or tear down
 * connections to the cloud.\n <i>Callback establishmeht methods</i>: Methods that may be used by strategies to announce their ability to serve a
 * certain cloud or peer connection functionality.
 */
class BitTorrentClient : public Application
{
  ...
  private:
    ...
    std::string m_streamHash;          // Reference to the global Torrent instance
    ...
    std::string m_bitfieldFillType;  // A string indicating the way the bitfield of this client instance is filled at start (i.e., download status).
                                     // Using this after initialization in StartApplication() ins meaningless.
    std::map<uint32_t, uint8_t> m_bitfieldManipulations; // Can be used to edit the bitfield after pre-filling it. Using this after initialization in
                                                         // StartApplication() ins meaningless.
    ...
    // Time                                 m_postPieceTimeoutPatience;   // A currently unused attribute for a work-in-progress heuristic in the base
    ...
    std::map<std::string, std::pair<std::string, std::string>>
        m_lastChangedStrategyOptions; // Provides the settings delta for the strategies. Settings in GetOptions() are the same but also include
    ...
    // InfoHash, peer
    std::map<Ptr<Peer>, std::set<std::string>>
  ...
  private:
    ...
    std::list<Callback<void, Ptr<Peer>>>
        m_establishedEventListeners; // After a connection has been established. Bitfield not yet ready, communication instable.
    ...
    std::list<Callback<void, std::string>> m_streamBufferReadyEventListeners;
    std::list<Callback<void, std::string>> m_getSeederEventListeners;
    ...
    std::map<uint8_t, std::list<Callback<void, Ptr<Peer>, const std::string&>>>
        m_extensionMessageListeners; // Can be used for arbitrary additional messages according to BEP 10.
    ...
    std::list<Callback<void>>
        m_strategyOptionsChangedEventListeners; // Each strategy with attributes that may be changed at run-time should register to this
...
public:
  ...
  public:
    ...
    const std::string GetStreamHash() const
    {
        return m_streamHash;
    }
    void SetStreamHash(std::string streamHash);
     ...
     * (any other setting) for a bitfield filled up to a certain point from the left, with a tail of randomly-set entries to simulate a
     * Video-on-Demand client during playback.
     *
    ...
    /**
     * @returns the client's chosen identifier within the swarm. It is composed of the word "VODSim", a dash, the ID of the Node the Application is
     * installed on, another dash and a random sequence.
     */
     ...
     *
     * @param interfaceId the ID of the interface the client should use. It automatically derives and updates the IP address used. Default is the
     * first interface of the node.
     */
     ...
     *
     * Use this method to make the BitTorrent client open a specific port for incoming connections if you have installed multiple clients on one
     * specific or interface.
     *
     ...
     * chunks.
     *
     * Caution: Having this setting not aligned with the might break certain strategy implementations, such as the default one provided by the
    ...
    void RegisterUpperStream(Ptr<Peer> peer, std::string streamHash);
    ...
    void GetSeederEvent(std::string streamHash);

    std::set<Ptr<Peer>>::const_iterator GetSubscriptionListIterator(std::string streamHash) const;

    /**
     * @returns an iterator to the end of the list of peers.
     */
    std::set<Ptr<Peer>>::const_iterator GetSubscriptionListEnd(std::string streamHash) const;

    // std::set<Ptr<Peer>>::const_iterator GetSubscriptionListEnd() const;
    /**
  ...
  public:
     ...
     *
     * This nay happen, e.g., after the server for incoming connections was torn down. Note that this does not indicate that a connection to a peer
     * was closed.
     *
     * The current cloud connection status can be obtained using the GetConnectedToCloud and GetConnectionToCloudSuspended methods.
     */
    ...
    /**
     * \brief This event is triggered upon the reception of a message of the <a href="http://www.bittorrent.org/beps/bep_0010.html"
     * target="_blank">BitTorrent Extension Protocol (BEP 10)</a>.
     *
     * @param messageType the type of the message received. 0 Indicates a Extension Message Protocol handshake, > 0 some message negotiated during the
     * handshake.
     ...
     *
     * This method also triggers the announcement of the peer as a seeder with the peer discovery mechanism (see the TriggerCallbackAnnounceAsSeeder()
     * method) and initiates the timeout until the newly-seeding client leaves the cloud (see the SetSeedingDuration() method).
     *
  ...
  public:
    ...
    void RegisterCallbackStreamBufferReadyEvent(Callback<void, std::string> eventCallback);
    void UnregisterCallbackStreamBufferReadEvent(Callback<void, std::string> eventCallback);

    void RegisterCallbackGetSeederEvent(Callback<void, std::string> eventCallback);
    void UnregisterCallbackGetSeederEvent(Callback<void, std::string> eventCallback);
    // Callback triggers
  public:
    // Cloud connection
