...
const std::set<std::pair<uint32_t, uint16_t>>&
PeerConnectorStrategyBase::GetPotentialClients() const
{
    return m_potentialClients;
}

Time
PeerConnectorStrategyBase::GetPeriodicInterval() const
{
    return m_periodicInterval;
}

void
PeerConnectorStrategyBase::SetPeriodicInterval(Time periodicInterval)
{
...
Time
PeerConnectorStrategyBase::GetReannouncementInterval() const
{
    return m_reannouncementInterval;
}

void
PeerConnectorStrategyBase::SetReannouncementInterval(Time reannouncementInterval)
{
...
Time
PeerConnectorStrategyBase::GetTimeoutPeriod() const
{
    return m_timeout;
}

void
PeerConnectorStrategyBase::SetTimeoutPeriod(Time timeoutPeriod)
{
    m_timeout = timeoutPeriod;
}

uint16_t
PeerConnectorStrategyBase::GetPeerCount() const
{
    return m_connectedTo.size() - 1 + m_pendingConnections.size();
}

void
PeerConnectorStrategyBase::ConnectToCloud()
{
...
void
PeerConnectorStrategyBase::AnnounceAsSeeder()
{
...
void
PeerConnectorStrategyBase::DisconnectFromCloud()
{
...
void
PeerConnectorStrategyBase::StartListening(uint16_t port)
{
    ...
    m_serverSocket->SetAcceptCallback(MakeCallback(&PeerConnectorStrategyBase::AcceptCallback, this),
                                      MakeCallback(&PeerConnectorStrategyBase::NewConnectionCreatedCallback, this));
...
void
PeerConnectorStrategyBase::DisconnectPeer(Ptr<Peer> peer)
{
    peer->CloseConnection(false); // false => NOT slient, i.e., issue events to the client and associated strategies
}

void
PeerConnectorStrategyBase::DisconnectPeerSilent(Ptr<Peer> peer)
{
    peer->CloseConnection(true);
}

void
PeerConnectorStrategyBase::ConnectToPeer(Ipv4Address address, uint16_t port)
{
...
uint16_t
PeerConnectorStrategyBase::ConnectToPeers(uint16_t count)
{
...
void
PeerConnectorStrategyBase::DisconnectPeers(int32_t count)
{
    if (count == -1) // Close all connections
    {
        ...
        {
            Simulator::Schedule(MilliSeconds(1), &PeerConnectorStrategyBase::DisconnectPeer, this, it->first);
        }
    }
    else
    {
...
uint8_t
PeerConnectorStrategyBase::AcceptConnection(Ptr<Socket> socket, Address addr)
{
...
void
PeerConnectorStrategyBase::AddConnection(uint32_t address)
{
    m_connectedTo.insert(address);
}

void
PeerConnectorStrategyBase::DeleteConnection(uint32_t address)
{
...
void
PeerConnectorStrategyBase::ProcessPeriodicSchedule()
{
    if (m_myClient->GetDownloadCompleted())
    {
        return;
...
void
PeerConnectorStrategyBase::ProcessPeriodicReannouncements()
{
...
void
PeerConnectorStrategyBase::ParseResponse(std::istream& response)
{
...
bool
PeerConnectorStrategyBase::AcceptCallback(Ptr<Socket> sock, const Address& addr)
{
    ...
    if (ok)
    {
        ...
        if (ok)
        {
            NS_LOG_INFO("PeerConnectorStrategyBase: " << m_myClient->GetIp() << ": Accepted connection with "
...
void
PeerConnectorStrategyBase::NewConnectionCreatedCallback(Ptr<Socket> sock, const Address& addr)
{
...
void
PeerConnectorStrategyBase::CheckAndDisconnectIfRejected(Ptr<Peer> peer)
{
...
void
PeerConnectorStrategyBase::TrackerResponseEvent(Ptr<Socket> socket)
{
...
void
PeerConnectorStrategyBase::TrackerTimeout(TrackerContactReason reason, uint16_t numwant, bool closeCurrentConnection)
{
...
bool
PeerConnectorStrategyBase::ContactTracker(TrackerContactReason event,
                                          uint16_t numwant,
                                          std::map<std::string, std::string> additionalParameters,
                                          bool closeCurrentConnection)
{
    ...
    if (m_httpCC.HttpGetRequest(m_myClient->GetNode(),
                                ...
                                80,
                                request.str(),
                                MakeCallback(&PeerConnectorStrategyBase::TrackerResponseEvent, this)))
    {
...
bool
PeerConnectorStrategyBase::ContactTrackerWrapper(TrackerContactReason reason, uint16_t numwant, bool closeCurrentConnection)
{
    std::map<std::string, std::string> additionalParameters;
...
void
PeerConnectorStrategyBase::CancelCurrentTrackerRequest()
{
    Simulator::Cancel(m_timeoutEvent);
    m_httpCC.CloseAndReInit();
}
...
void
PeerConnectorStrategyBase::ProcessPeerConnectionEstablishedEvent(Ptr<Peer> peer)
{
...
void
PeerConnectorStrategyBase::ProcessPeerConnectionFailEvent(Ptr<Peer> peer)
{
    m_pendingConnections.erase(peer->GetRemoteIp().Get());
    if (!m_myClient->GetDownloadCompleted())
    {
...
void
PeerConnectorStrategyBase::ProcessConnectionCloseEvent(Ptr<Peer> peer)
{
    DeleteConnection(peer->GetRemoteIp().Get()); // Already deletes only 1 entry from the multimap
}

void
PeerConnectorStrategyBase::ProcessPeerBitfieldReceivedEvent(Ptr<Peer> peer)
{
    ...
    if (m_myClient->GetDownloadCompleted())
    {
        ...
        if (completed && m_myClient->GetIp().Get() < peer->GetRemoteIp().Get())
        {
            ...
            NS_LOG_INFO("PeerConnectorStrategyBase: " << m_myClient->GetIp() << ": Closing attempted connection from also finished peer "
...
void
PeerConnectorStrategyBase::ProcessDownloadCompleteEvent()
{
    ...
    for (std::map<Ptr<Peer>, std::set<std::string>>::const_iterator it = m_myClient->GetPeerListIterator(); it != m_myClient->GetPeerListEnd(); ++it)
    {
        ...
        if (completed)
        {
            ...
            Simulator::Schedule(Seconds(5), &PeerConnectorStrategyBase::DisconnectPeer, this, it->first);
