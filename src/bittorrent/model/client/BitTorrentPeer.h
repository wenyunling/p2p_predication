 ...
 *
 * All default BitTorrent Peer Wire Protocol messages as defined in the <a href="http://wiki.theory.org/BitTorrentSpecification#Messages"
 * target="_blank">Bittorrent Protocol Specification v1.0</a> are supported. Additionally, you can send messages adhering to the <a
 * href="http://www.bittorrent.org/beps/bep_0010.html" target="_blank">BitTorrent Extension Protocol (BEP 10)</a>.
 *
 * Note: The sending and analysis of a BitTorrent Extension Protocol handshake message are currently not implemented. However, you may still use the
 * SendExtendedMessage member function to send messages other than those defined in the standard Peer Wire Protocol specification.
 */
class Peer : public Object
{
  ...
  private:
    ...
    std::set<std::string> m_remoteHashInfo; // The Infohash subscribed/transmit by the remote peer
    ...
    std::list<RequestInformation>
        m_requestQueue; // The REQUESTs we have to fulfill for the other peer (NOTE: Requests are NOT automatically added here!)
    ...
    bool m_blockSendingActive; // Whether we are currently sending out a PIECE or whether we are "idle" in the sense of being able to transmit some
                               // other message
  ...
  public:
     ...
     *
     * Note: The connection is ready after the bitfield of the remote was received; the class will issue a PeerBitfieldReceivedEvent event if this
     * message is received. Prior to that, you should not use any of the communication methods provided by the class. Note that this behavior is not
     * 100% compliant with the specification, as sending the bitfield message is optional if a client has no pieces. This may lead to some clients not
     * being accepted as peers.
     *
     ...
     *
     * Note: The message is only sent when it is needed, i.e., an (UN)INTERESTED message is not repeated if the current client state is already
     * (un)interested.
     *
     ...
     *
     * This method creates and inserts an <a href="http://www.bittorrent.org/beps/bep_0010.html" target="_blank">BitTorrent Extension Protocol (BEP
     * 10)</a> message into the send queue. The created message has a Peer Wire message ID of 20 ("Extension Protocol") and the payload can be
     * arbitrary.
     *
     ...
     * so different sent messages can be distinguished without a global message registry. At the moment, there is no function to generate this
     * message, but if existant, it can also be sent using this method.
     *
  ...
  public:
    ...
    /**
     * @returns the infoHash of the remote peer.
     */
    std::set<std::string> GetRemoteSubscribeInfoHash() const;
