...
ChokeUnChokeStrategyBase::ChokeUnChokeStrategyBase(Ptr<BitTorrentClient> myClient)
...
void
ChokeUnChokeStrategyBase::DoInitialize()
{
...
Time
ChokeUnChokeStrategyBase::GetPeriodicInterval() const
{
    return m_periodicInterval;
}

void
ChokeUnChokeStrategyBase::SetPeriodicInterval(Time periodicInterval)
{
...
uint32_t
ChokeUnChokeStrategyBase::GetOptimisticMultiple() const
{
    return m_optimisticMultiple;
}

void
ChokeUnChokeStrategyBase::SetOptimisticMultiple(uint32_t optimisticMultiple)
{
...
void
ChokeUnChokeStrategyBase::ProcessBitfieldReceivedEvent(Ptr<Peer> peer)
{
...
void
ChokeUnChokeStrategyBase::ProcessPeerInterestedChangingEvent(Ptr<Peer> peer)
{
    ...
    if (peer->IsInterested())
    {
        ...
        // If less than the maximum of clients is currently unchoked - and this one is interested in being unchoked - trigger the periodic event
        // immediately
...
void
ChokeUnChokeStrategyBase::ProcessPeerConnectionCloseEvent(Ptr<Peer> peer)
{
...
void
ChokeUnChokeStrategyBase::ProcessPeriodicSchedule()
{
    ...
    if ((bool)m_optimisticallyUnchokedPeer && m_periodicScheduleRound % m_optimisticMultiple == 0)
    {
    ...
    if ((bool)worstDownloadingPeer.second)
    {
    ...
    if (m_periodicScheduleRound % m_optimisticMultiple == 0) // Only optimistically unchoke every m_optimisticMultiple rounds
    {
        ...
        if (newPeerChooser < 2 &&
            m_newlyConnectedPeers.size() >
                0) // but since they may also be included in the list of "interested" peers, we only need to check for a "2" instead of a "3" here
        {
        ...
        // If not using the newly connected peers (or if no interested newly connected peer is found), use one of the normal interested peers for
    ...
    {
        NS_LOG_INFO("ChokeUnChokeStrategyBase: Choked " << (*it)->GetRemoteIp() << " (connected for "
                                                        << (Simulator::Now() - (*it)->GetConnectionEstablishmentTime()).GetSeconds() << " seconds)"
                                                        << "; upload rate: " << (*it)->GetBpsUpload() << ", download rate: "
                                                        << (*it)->GetBpsDownload() << "; interested: " << (*it)->IsInterested());
    ...
    {
        NS_LOG_INFO("ChokeUnChokeStrategyBase: Unchoked "
                    << (*it)->GetRemoteIp() << " (connected for " << (Simulator::Now() - (*it)->GetConnectionEstablishmentTime()).GetSeconds()
                    << " seconds)"
                    << "; upload rate: " << (*it)->GetBpsUpload() << ", download rate: " << (*it)->GetBpsDownload()
                    << "; interested: " << (*it)->IsInterested() << "; optimistically unchoked: " << ((*it) == newOptimisticallyUnchokedPeer));
...
std::pair<double, Ptr<Peer>>
ChokeUnChokeStrategyBase::GetCandidateForRemoval() const
{
...
std::pair<double, Ptr<Peer>>
ChokeUnChokeStrategyBase::GetCandidateForInsertion() const
{
...
ChokeUnChokeStrategyBase::ProcessGatherMetricsEvent()
{
