...
{
    if (m_connectionState != CONN_STATE_NOT_CONNECTED) // Only allow this directly if we haven't
                                                       // initialized a connection before
    {
        return;
    }
...
void
Peer::ServeNewPeer(Ptr<Socket> socket, Ipv4Address address, uint16_t port)
{
    if (m_connectionState != CONN_STATE_NOT_CONNECTED) // Only allow this directly if we haven't
                                                       // initialized a connection before
    {
    ...
    // Step 3: Since the other client is already initialized (remote establishment of connection),
    // we await the BitTorrent handshake packet and prepare to send ours -->
    ...
    std::memcpy(peerId,
                m_myClient->GetPeerId().c_str(),
                std::min(static_cast<size_t>(BT_PROTOCOL_MESSAGES_HANDSHAKE_PEERID_LENGTH_MAX), m_myClient->GetPeerId().size()));
    ...
    // <-- Step 3: Since the other client is already initialized (remote establishment of
    // connection), we await the BitTorrent handshake packet and prepare to send ours
...
void
Peer::SetAmInterested(bool amInterested)
{
...
// NOTE: Since all message generation methods are organized more or less equally, this method is
// annotated exemplarily
void
Peer::RequestPiece(uint32_t pieceIndex, uint32_t blockOffSet, uint32_t blockLength)
{
    // Step 0: Only send out something, if the connection is not yet closed (due to concurrency
    // within the strategies and the simulation)
    ...
    // Step 2: Create a BitTorrentLengthHeader so we can read out the length of the message properly
    // again at the remote Peer
    BitTorrentLengthHeader lenHead(BT_PROTOCOL_MESSAGES_REQUEST_LENGTH);

    // Step 3: Create a BitTorrentTypeHeader so we can read out the type of the message properly
    // again at the remote Peer
...
void
Peer::CancelRequest(uint32_t pieceIndex, uint32_t blockOffSet, uint32_t blockLength)
{
    if (m_connectionState != CONN_STATE_CONNECTED)
    {
        return;
    ...
    // Prioritized sending
    if (m_blockSendingActive) // Insert the cancel request right at the beginning but after the
                              // currently sending block
    {
...
void
Peer::SendHaveMessage(uint32_t pieceIndex)
{
    if (m_connectionState != CONN_STATE_CONNECTED)
    {
        return;
    ...
    // Prioritized sending
    if (m_blockSendingActive) // Insert the have message right at the beginning but after the
                              // currently sending block
    {
        std::list<Ptr<Packet>>::iterator sendQueueIt = m_sendQueue.begin();
        std::list<bool>::iterator sendQueuePieceMessageIndicatorsIt = m_sendQueuePieceMessageIndicators.begin();
...
void
Peer::SetAmChoking(bool amChoking)
{
...
void
Peer::SendBlock(uint32_t pieceIndex, uint32_t blockOffSet, uint32_t blockLength)
{
...
Ipv4Address
Peer::GetRemoteIp() const
{
    return m_remoteIp;
}

uint16_t
Peer::GetRemotePort() const
{
    return m_remotePort;
}

std::string
Peer::GetRemotePeerId() const
{
...
bool
Peer::IsInterested() const
{
    return m_peerInterested;
}

const uint8_t*
Peer::GetPieceCorruptionMap() const
{
    return m_pieceCorruptionMap;
}

Time
Peer::GetConnectionEstablishmentTime() const
{
    return m_connectionEstablishmentTime;
}

uint64_t
Peer::GetTotalBytesDownloaded() const
{
    return m_totalBytesDownloaded;
}

uint64_t
Peer::GetTotalBytesUploaded() const
{
...
void
Peer::NotifyPeerOfInterestedChange()
{
    if (m_connectionState != CONN_STATE_CONNECTED)
    {
        return;
...
void
Peer::HandleCancel(Ptr<Packet> packet)
{
    ...
    while (!requestFound && it != m_sendQueuePieceMessageIndicators.end() && it3 != m_requestQueue.end())
    {
        if (*it) // If we have found a scheduled PIECE message
        {
            ...
            else // ... else, we know that this PIECE message is not what we have to cancel. We go
                 // on to the next one.
            {
                ++it3;
            }
...
bool
Peer::HandlePiece(Ptr<Packet> packet, uint32_t packetLength)
{
...
Peer::HandleRead(Ptr<Socket> socket)
{
    ...
    while (m_packetBuffer->GetSize() >= m_lengthHeader.GetPacketLength())
    {
        if (m_connectionState == CONN_STATE_AWAIT_HANDSHAKE)
        {
             ...
             * down here -------------------+ (Just as with BitTorrentLengthHeader, but 1 byte
             * instead of 4)
             *                              |
             *                              v
             */
        ...
        else if (m_connectionState == CONN_STATE_CONNECTED)
        {
            if (!m_lengthHeaderRemoved) // We have not yet inspected the beginning of the next message
            {
                ...
                {
                    m_lengthHeader.SetPacketLength(
                        BT_PROTOCOL_MESSAGES_LENGTHHEADER_LENGTH); // So we will end up in this same else if(!m_lengthHeaderRemoved) section again
                }
                else
                {
                    // If we receive messages >
                    // BT_PROTOCOL_MESSAGES_LENGTHHEADER_THRESHOLD_PIECE_OR_EXTENSIONPROTOCOL_MESSAGE
                    // bytes, we assume they are piece messages and start estimating the upload
                    // bandwidth of the peer Note: Since we do this all the time, it does not matter
                    // that a bitfield message might be sent; the time is just replaced
            ...
            else
            {
                ...
                m_lengthHeader.SetPacketLength(BT_PROTOCOL_MESSAGES_LENGTHHEADER_LENGTH); // So we will stay in / enter this while loop if we have
                                                                                          // enough bytes in the buffer left
            }
        }
    }
}
...
void
Peer::HandleSend(Ptr<Socket> socket, uint32_t bytesFree)
{
...
void
Peer::HandleDataSent(Ptr<Socket> socket, uint32_t dataSent)
{
    uint64_t currentSecond = static_cast<uint64_t>(Simulator::Now().GetSeconds());
    uint64_t currentSecondModulo = currentSecond % BT_PEER_DOWNLOADUPLOADRATE_ROLLING_AVERAGE_SECONDS;
...
void
Peer::HandleConnected(Ptr<Socket> socket)
{
    ...
    // When we are connected, we send out the handshake packet and start waiting for the packet of
    // our peer
    m_connectionState = CONN_STATE_AWAIT_HANDSHAKE;
...
void
Peer::HandleConnectionClosedWithError(Ptr<Socket> socket)
{
...
void
Peer::PseudoDeInitializeMePhase2()
{
