...
PartSelectionStrategyBase::PartSelectionStrategyBase(Ptr<BitTorrentClient> myClient)
    : AbstractStrategy(myClient)
{
    // Step 1: Check if the download is completed already completed since then, no other work has to
    // be done here
     ...
     *
     * NOTE: For the sake of simplicity, this implementation will not store whether parts of a pice
     * have been downloaded over the course of multiple client instantiations; i.e., while the
     * download progress of a single piece is stored during a single session (i.e., an ns-3
     * simulation run), storing this sub-piece download state is not possible.
     */
    InitializeNeededPiecesMap();

    // Step 5: Retrieve the number of blocks per piece so we can more correctly determine the
    // time-outs of block requests
...
void
PartSelectionStrategyBase::DoInitialize()
{
    ...
    /*
     * Note: DO NOT START THE SCHEDULER RIGHT NOW! This will be done automatically when connection
     * to a peer has been fully established!
     */
}
...
{
    /*
     * This is a little tricky a change: We have to immediately cancel ALL requests sent out to any
     * peer and re-initialize the list of possible requests. NOTE: This will result in a multitude
     * of requests being not fulfilled, especially: Any already-begun piece will be emptied again
     * since keeping track of all half-finished requests in a non-uniform-size fashion is too
     * complicated!
     */
    if (m_myClient->GetLastChangedStrategyOptionName() == "request_block_size")
    {
        ...
        // Step 2: Clean the requested blocks map (should be in sync with the needed pieces map and
        // hence now only contain invalid pointers
        ...
        // Step 4: Retrieve the number of blocks per piece so we can more correctly determine the
        // time-outs of block requests
        NeededPiecesMap::iterator it = m_neededPieces.begin();
        if (it != m_neededPieces.end())
        {
...
Time
PartSelectionStrategyBase::GetPeriodicInterval() const
{
    return m_periodicInterval;
}

void
PartSelectionStrategyBase::SetPeriodicInterval(Time periodicInterval)
{
...
PartSelectionStrategyBase::PieceNeeded
PartSelectionStrategyBase::GeneratePieceNeeded(uint32_t pieceNeeded)
{
    ...
    uint32_t pieceLength = (m_myClient->GetTorrent()->HasTrailingPiece() && pieceNeeded == m_myClient->GetTorrent()->GetNumberOfPieces() - 1)
...
void
PartSelectionStrategyBase::InitializeNeededPiecesMap()
{
    // Step 0: Clear our list of needed pieces
    m_neededPieces.clear();

    // Step 1: Iterate through our client's bitfield and check each entry for missing pieces, create
    // an entry in the map if missing
    ...
    {
        // We invert the current bitfield entry and then check bitwise; using a left shift operation
        // after each check
...
void
PartSelectionStrategyBase::RequestBlock(BlockRequested* block)
{
    NS_LOG_INFO("Requesting block " << (*block).m_pieceIndex << "@" << (*block).m_blockOffset << "->"
                                    << (*block).m_blockOffset + (*block).m_blockLength << " from peer " << (*block).m_requestedFrom->GetRemoteIp()
                                    << "; timeout at " << (*block).m_timeoutTime.GetMilliSeconds() << "ms.");
...
void
PartSelectionStrategyBase::CancelRequest(BlockRequested block)
{
    NS_LOG_INFO("Cancelling block " << block.m_pieceIndex << "@" << block.m_blockOffset << "->" << block.m_blockOffset + block.m_blockLength
                                    << " from peer " << block.m_requestedFrom->GetRemoteIp() << ".");
    ...
    RemoveRequest(block,
                  false); // false since we have already canceled it in step 1 (more intuitive)
...
void
PartSelectionStrategyBase::SaveRequest(BlockRequested* block)
{
...
void
PartSelectionStrategyBase::RemoveRequest(BlockRequested block, bool cancel)
{
    ...
    // Step 3: Iterate through the pending blocks and remove the reference to the block given as the
    // argument
    while (pbIt != pendingBlocks.end())
    {
        ...
        if (block == (**pbIt))
        {
            // Step 3a: Delete the reference to the object in the list of open requests for the peer
            // it was assigned to
            ...
            // Step 3c: Remove the canceled request's peer from the data structure keeping track of
            // them
...
void
PartSelectionStrategyBase::RemoveAllRequests(BlockRequested block, bool cancel)
{
    // Step 1: Get a reference to the piece this block belongs to
    NeededPiecesMap::iterator npmIt = m_neededPieces.find(block.m_pieceIndex);
    if (npmIt == m_neededPieces.end())
    ...
    // Step 3: Iterate through the pending blocks and remove all references to the block given as
    // the argument
    while (pbIt != pendingBlocks.end())
    {
        if (block.requestEqualTo(**pbIt))
        {
            ...
            // Step 3c: Delete the reference to the object in the list of open requests for the peer
            // it was assigned to
            RequestedBlocksMap::iterator rbmIt = m_requestedBlocks.find(block.m_requestedFrom);
            if (rbmIt != m_requestedBlocks.end())
            {
            ...
            // Step 3e: Remove the cancelled request's peer from the data structure keeping track of
            // them
            std::multiset<Ptr<Peer>>::iterator it = (*npmIt).second.m_requestedFrom.find((**pbIt).m_requestedFrom);
            (*npmIt).second.m_requestedFrom.erase(it);
...
void
PartSelectionStrategyBase::ProcessBitfieldReceivedEvent(Ptr<Peer> peer)
{
...
void
PartSelectionStrategyBase::ProcessPeerChokeChangingEvent(Ptr<Peer> peer)
{
...
void
PartSelectionStrategyBase::ProcessPeerHaveEvent(Ptr<Peer> peer, uint32_t pieceIndex)
{
...
void
PartSelectionStrategyBase::ProcessPeerBlockCompleteEvent(Ptr<Peer> peer, uint32_t pieceIndex, uint32_t blockOffset, uint32_t blockLength)
{
    ...
    // Step 1a: If we have not found the block, we have not requested it (from this peer) and drop
    // the message
    ...
    // Step 2: Remove all instances to the block in all our data structures and cancel all pending
    // requests
    RemoveAllRequests(block, true);

    // Step 3: Now, mark this block as received by removing its entry from the list of "possible
    // blocks"
    ...
    // Step 4: If our request information is empty and the piece has been successfully downloaded
    // (checksum), mark the whole piece as finished
    if ((*npmIt).second.m_possibleBlocks.empty())
    {
        ...
        if (pieceOk)
        {
            ...
            // Step 4.2a3: Issue an event indicating the completion of this piece by the sending
            // peer
            m_myClient->PieceCompleteEvent(peer, pieceIndex);
        }
        else // Step 4.2b: In case the piece was not successfully downloaded, re-enter all blocks
             // into the data structure for neeeded blocks
        {
...
void
PartSelectionStrategyBase::ProcessPeerConnectionCloseEvent(Ptr<Peer> peer)
{
    // A closed connection more or less is the same as a choked connection. Plus: We delete the
    // references to this peer totally
...
void
PartSelectionStrategyBase::ProcessRequestTimeout(Ptr<Peer> peer, uint32_t pieceIndex, uint32_t blockOffset, uint32_t blockLength)
{
    // Step 1: Remove the request information for this block, cancelling the piece with the remote
    // peer
...
void
PartSelectionStrategyBase::ProcessCompletedPiece(uint32_t pieceIndex)
{
...
void
PartSelectionStrategyBase::CheckDownloadCompleted()
{
...
bool
PartSelectionStrategyBase::RequestAllowedForBlock(Ptr<Peer> peer, uint32_t pieceIndex, uint32_t blockOffset, uint32_t blockLength)
{
    /*
     ...
     * 4) The number of already-pending requests for this piece for the given peer must not be
     * exceeded 5) The number of already-pending requests for this particular block must not be
     * exceeded (!= criterion 2!)
     */
...
void
PartSelectionStrategyBase::GetHighestPriorityBlockForPeer(Ptr<Peer> peer, BlockRequested& blockPtr)
{
    // Step 1: Prepare the passed block so far we can so that accessing it after this function does
    // not result in undesired behavior
    blockPtr.m_requestedFrom = peer;
    blockPtr.m_blockLength = 0;

    // Step 2: Calculate the timeout of a piece by taking into account how many requests are already
    // running for this peer
    blockPtr.m_timeoutTime =
        Simulator::Now() + MilliSeconds((1 + m_requestedBlocks[peer].size()) * m_myClient->GetPieceTimeout().GetMilliSeconds() / m_blocksPerPiece);
...
PartSelectionStrategyBase::Scheduler()
{
    ...
    for (std::list<uint32_t>::const_iterator it = requestOrder.begin(); it != requestOrder.end(); ++it)
    {
        ...
        while (requestedBlocks.size() < m_myClient->GetMaxRequestsPerPeer())
        {
            ...
            {
                // Step 3a1a: If the peer is choking us, the only thing we can do is announce our
                ...
                (*block).m_timeoutEvent = Simulator::Schedule((*block).m_timeoutTime - Simulator::Now(),
                                                              &PartSelectionStrategyBase::ProcessRequestTimeout,
                                                              this,
                                                              currentPeer,
                                                              (*block).m_pieceIndex,
                                                              (*block).m_blockOffset,
            ...
            // Step 3a2: Else, we break looking for further available blocks and, if needed,
            // signalize no interest to the peer
            else
            {
                // Step 3a2a: Delete the block
                delete block;

                // Step 3a2b: If there are no reuqested blocks for this peer, set our status to
                // uninterested (we have no requests left)
...
inline std::list<uint32_t>
PartSelectionStrategyBase::GetPeerOrderForScheduler()
{
    return Utilities::GetPermutationP(m_myClient->GetActivePeers().size(), m_myClient->GetActivePeers().size());
}

void
PartSelectionStrategyBase::ProcessPeriodicSchedule()
{
    if (!m_myClient->GetDownloadCompleted())
    {
        bool goOn = true;

        // If we haven't scheduled the disconnection of a peer, we can start our piece selection
        // scheduler
        if (goOn)
        {
            Scheduler();
        }
        else // else, we delay the execution of the scheduler until the disconnections have been
             // completed
        {
            /*
...
void
PartSelectionStrategyBase::DebugPrint()
{
    ...
    while (npmIt != m_neededPieces.end())
    {
        ...
        {
            std::cout << "				" << (**it2).m_pieceIndex << "@" << (**it2).m_blockOffset << "->"
                      << (**it2).m_blockOffset + (**it2).m_blockLength << " from " << (**it2).m_requestedFrom->GetRemoteIp() << std::endl;
            ++it2;
        }
    ...
    while (rbmIt != m_requestedBlocks.end())
    {
        ...
        {
            std::cout << "				" << (**it).m_pieceIndex << "@" << (**it).m_blockOffset << "->" << (**it).m_blockOffset + (**it).m_blockLength
                      << " from " << (**it).m_requestedFrom->GetRemoteIp() << std::endl;
            ++it;
        }
