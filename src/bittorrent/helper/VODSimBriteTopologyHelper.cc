...
NS_LOG_COMPONENT_DEFINE("bittorrent::VODSimBriteTopologyHelper");
NS_OBJECT_ENSURE_REGISTERED(VODSimBriteTopologyHelper);

TypeId
VODSimBriteTopologyHelper::GetTypeId(void)
{
    static TypeId tid = TypeId("ns3::VODSimBriteTopologyHelper").SetParent<Object>();
    return tid;
}

VODSimBriteTopologyHelper::VODSimBriteTopologyHelper()
{
...
NodeContainer
VODSimBriteTopologyHelper::AddClientNodesToLastTopology(uint32_t numberOfClientNodes, std::string deviceType)
{
    ...
    for (uint32_t i = 0; i < numberOfClientNodes; ++i)
    {
        ...
        clientNodes.Add(AddNodeToLastTopology(parent,
                                              deviceType,
                                              (*bandwidthsIt).first,
                                              (*bandwidthsIt).second,
                                              delay,
...
NodeContainer
VODSimBriteTopologyHelper::AddNodeToLastTopology(Ptr<Node> parent,
                                                 std::string deviceType,
                                                 std::string upstreamBandwidth,
                                                 std::string downstreamBandwidth,
                                                 std::string delay,
                                                 bool isRouterNode,
                                                 bool isBorderRouterNode,
                                                 int32_t AS,
                                                 bool isTrackerNode,
...
NodeContainer
VODSimBriteTopologyHelper::AddTapNodeToLastTopology(std::string tapDeviceName,
                                                    Ptr<Node> parent,
                                                    std::string upstreamBandwidth,
                                                    std::string downstreamBandwidth,
                                                    std::string delay,
...
bool
VODSimBriteTopologyHelper::HasTapNode() const
{
    return m_hasTapNode;
}

bool
VODSimBriteTopologyHelper::EstablishNetworkOnLastTopology()
{
    ...
    for (linksIt = LinksBegin(); linksIt != LinksEnd(); ++linksIt, ++i)
    {
        ...
        if (link.GetAttribute("Device-Type") == "point-to-point" || link.GetAttribute("Device-Type") == "topology" ||
            link.GetAttribute("Device-Type") == "tracker")
        {
    ...
    ASSIGN_ADDRESSES(trackerInterfaces, "10.1.0.0", "255.255.0.0");
    ASSIGN_ADDRESSES(ptpInterfaces, "10.2.0.0", "255.255.0.0");
...
VODSimBriteTopologyHelper::GetBandwidthDistributionFromFile(uint32_t numberOfNodes)
{
    ...
    NS_LOG_INFO("VODSimBriteTopologyHelper: Bucket "
                << numberOfBuckets - 1 << ", own probability: " << static_cast<double>(bucketSizes[numberOfBuckets - 1]) / numberOfSamples << ".");
...
std::list<std::pair<std::string, std::string>>
VODSimBriteTopologyHelper::GetSimpleDSLDistribution(uint32_t numberOfNodes)
{
    std::list<std::pair<std::string, std::string>> result;
...
void
VODSimBriteTopologyHelper::SetBandwidthSamplesFile(std::string bandwidthSamplesFile)
{
    m_bandwidthSamplesFile = bandwidthSamplesFile;
}

std::string
VODSimBriteTopologyHelper::GetBandwidthSamplesFile() const
{
    return m_bandwidthSamplesFile;
}

void
VODSimBriteTopologyHelper::SetClientLinkDelays(uint32_t minDelay, uint32_t maxDelay)
{
...
uint32_t
VODSimBriteTopologyHelper::GetClientLinkDelaysMin() const
{
    return m_minDelay;
}

uint32_t
VODSimBriteTopologyHelper::GetClientLinkDelaysMax() const
{
    return m_maxDelay;
}

void
VODSimBriteTopologyHelper::EnablePcaps(std::string pcapFilePrefix)
{
    m_pcapFilePrefix = pcapFilePrefix;
}

std::string
VODSimBriteTopologyHelper::GetPcapFilePrefix() const
{
    return m_pcapFilePrefix;
}

void
VODSimBriteTopologyHelper::DistributeRouterNodesToMPISystems()
{
#ifdef NS3_MPI
    if (MpiInterface::GetSize() == 1)
    {
        NS_LOG_INFO("VODSimBriteTopologyHelper: MPI is not enabled. Performing all computations for routers on 1 system.");
        return;
    }
    else
    {
        NS_LOG_INFO("VODSimBriteTopologyHelper: Performing computations on " << MpiInterface::GetSize() << " systems.");
    }

    for (std::map<std::string, Ptr<Node>>::iterator it = m_routers.begin(); it != m_routers.end(); ++it)
    {
        ((*it).second)
            ->SetAttribute("SystemId",
                           UintegerValue(m_autonomousSystems[lexical_cast<std::string>(((*it).second)->GetId())] % MpiInterface::GetSize()));
        NS_LOG_INFO("VODSimBriteTopologyHelper: AS "
                    << m_autonomousSystems[lexical_cast<std::string>(((*it).second)->GetId())] << " routers execute on CPU "
                    << m_autonomousSystems[lexical_cast<std::string>(((*it).second)->GetId())] % MpiInterface::GetSize() << ".");
    }
    for (std::map<std::string, Ptr<Node>>::iterator it = m_borderRouters.begin(); it != m_borderRouters.end(); ++it)
    {
        ((*it).second)
            ->SetAttribute("SystemId",
                           UintegerValue(m_autonomousSystems[lexical_cast<std::string>(((*it).second)->GetId())] % MpiInterface::GetSize()));
        NS_LOG_INFO("VODSimBriteTopologyHelper: AS "
                    << m_autonomousSystems[lexical_cast<std::string>(((*it).second)->GetId())] << " border routers execute on CPU "
                    << m_autonomousSystems[lexical_cast<std::string>(((*it).second)->GetId())] % MpiInterface::GetSize() << ".");
    }
#endif
}
...
void
VODSimBriteTopologyHelper::DistributeClientNodesToMPISystems()
{
#ifdef NS3_MPI
    if (MpiInterface::GetSize() == 1)
    {
        NS_LOG_INFO("VODSimBriteTopologyHelper: MPI is not enabled. Performing all computations for normal nodes on 1 system.");
        return;
    }
    ...
    {
        ((*it).second)
            ->SetAttribute("SystemId", UintegerValue(m_clientsToRouters[lexical_cast<std::string>(((*it).second)->GetId())]->GetSystemId()));
        NS_LOG_INFO("VODSimBriteTopologyHelper: Node " << ((*it).second)->GetId() << " executes on CPU "
                                                       << m_clientsToRouters[lexical_cast<std::string>(((*it).second)->GetId())]->GetSystemId()
                                                       << ".");
    }
#endif
}
...
bool
VODSimBriteTopologyHelper::WriteLastTopologyToGraphVizFile(std::string fileName) const
{
    ...
    if (!file.is_open())
    {
        NS_ABORT_MSG("VODSimBriteTopologyHelper: Error: Could not open file \"output/" << fileName << ".dot\" for output. Aborting.");
        return false;
    }
