...
{
  TapBridgeHelper tapHelper;
  tapHelper.SetAttribute ("Mode", StringValue ("UseLocal"));
  for (uint32_t i = 0; i < tapNodes->GetN (); i++)
	{
	  tapHelper.SetAttribute ("DeviceName", StringValue (tapDeviceNames->front ()));
	  tapDeviceNames->pop_front ();
	  Ptr<Node> tapNode = tapNodes->Get (i);
	  tapHelper.Install (tapNode, tapNode->GetObject<CsmaNetDevice> ());
	}
}
