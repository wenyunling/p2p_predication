...
#ifndef VODSIM_BRITE_TOPOLOGY_HELPER_H__
#define VODSIM_BRITE_TOPOLOGY_HELPER_H__
 ...
 */
class VODSimBriteTopologyHelper : public BriteTopologyReader
{
// Fields
private:
  VODSimBriteTopologyHelper (const VODSimBriteTopologyHelper&);
  VODSimBriteTopologyHelper& operator= (const VODSimBriteTopologyHelper&);
...
public:
   ...
   *
   * Adds links between the nodes created by VODSimBriteTopologyHelper::Read() and
   * VODSimBriteTopologyHelper::AddClientNodesToLastTopology(). The links are created using the
...
#endif // VODSIM_BRITE_TOPOLOGY_HELPER_H__
