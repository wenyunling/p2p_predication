...
/*
 * Copyright (c) 2010-2015 ComSys, RWTH Aachen University
 *
...
#define SCHEDULE_CHAPTER_NOARGS(function, type)                                                                                                      \
    {                                                                                                                                                \
        ...
        {                                                                                                                                            \
            ...
            {                                                                                                                                        \
                Simulator::Schedule(MilliSeconds(time.GetMilliSeconds() + uv->GetInteger(0, time2.GetMilliSeconds() - time.GetMilliSeconds())),      \
                                    function,                                                                                                        \
                                    dynamic_cast<type*>(PeekPointer((*it)->GetApplication(0))));                                                     \
            }                                                                                                                                        \
        }                                                                                                                                            \
    }
#else
...
std::string
Story::GetSimulationId() const
{
    return m_simulationId;
}

bool
Story::GetLoggingToFile() const
{
    return m_loggingToFile;
}

bool
Story::GetUseFakeData() const
{
    return m_useFakeData;
}

bool
Story::GetCheckData() const
{
    return m_checkData;
}

uint32_t
Story::GetBTNodeCount() const
{
    return m_btNodeCount;
}

uint32_t
Story::GetOtherNodeCount() const
{
    return m_otherNodeCount;
}

std::string
Story::GetTorrentFolder() const
{
    return m_torrentFolder;
}

std::string
Story::GetTorrentFile() const
{
    return m_torrentFile;
}

std::string
Story::GetBandwidthSamplesFile() const
{
    return m_bandwidthSamplesFile;
}

std::string
Story::GetReplacement(std::string variableName)
{
    return m_variables[variableName];
}

void
Story::SetReplacement(std::string variableName, std::string value)
{
    m_variables[variableName] = value;
}

void
Story::ParseReplacements(std::string toParse)
{
    toParse += "/";
...
bool
Story::GetTrackerAdded() const
{
    return m_trackerAdded;
}

bool
Story::GetRandomSeedSet() const
{
    return m_randomSeedSet;
}

void
Story::ParseVideoTime(std::istringstream& lineBuffer, int32_t& result, bool& isRelative, uint32_t currentLine = 0)
{
    std::string buffer;
    lineBuffer >> buffer;
...
void
Story::ReadAndScheduleStory(std::string filePath, uint32_t simulationDuration)
{
    ...
    while (!storyFile.eof())
    {
        ...
        if (poisson || lineBuffer.str().substr(0, 4) == "from")
        {
            ...
            if (time < oldTime)
            {
                NS_ABORT_MSG(
                    "[line " << currentLine
                             << "] Error: You are scheduling an event that takes place before already scheduled events. That's not allowed.");
            }
            ...
            if (buffer != "till" && buffer != "until")
            {
                NS_ABORT_MSG(
                    "[line "
                    << currentLine
                    << "] Error: When defining a time range, you must provide the end of the range following the statements \"till\" or \"until\".");
            }
            ...
            if (time2 < time)
            {
                NS_ABORT_MSG("[line " << currentLine
                                      << "] Error: When defining a time range, the upper bound of the range must actually be an UPPER bound.");
            }
            ...
            if (poisson)
            {
                ...
                {
                    NS_ABORT_MSG("[line " << currentLine
                                          << "] Error: When defining a poisson process, you must provide the mean interarrival time following the "
                                             "statement \"interarrival\".");
                }
                ...
                {
                    NS_ABORT_MSG("[line " << currentLine
                                          << "] Error: When defining a poisson process, the interarrival time must be positive and not be larger "
                                             "than the difference between the start and end time of the process.");
                }
            }
        }
        else
        ...
        {
            std::cout << "Between " << time.GetMilliSeconds() << " and " << time2.GetMilliSeconds()
                      << " msec at a random point in time: " << std::endl;
        }
        else if (poisson)
        {
            std::cout << "Between " << time.GetMilliSeconds() << " and " << time2.GetMilliSeconds()
                      << " msec following a poisson process with an interarrival time of " << poissonInterarrivalTime.GetMilliSeconds()
                      << " msec: " << std::endl;
        }
        ...
        else if (target == "clients")
        {
            ...
            std::cout << "	For clients " << clientNrStart << " (" << m_btNodes->Get(clientNrStart - 1)->GetId() << ") - " << clientNrEnd << " ("
                      << m_btNodes->Get(clientNrEnd - 1)->GetId() << "):" << std::endl;
        }
        else if (target == "all")
        {
            lineBuffer >> target;
            ...
            else if (buffer == "leave")
            {
                ...
                if (buffer == "group")
                {
                    ...
                    else
                    {
                        ...
                        std::cout << "		Scheduled leaving of group " << buffer << " which now has " << m_groups[buffer].GetN() << " nodes"
                                  << std::endl;
                    }
                }
            ...
            else if (buffer == "set")
            {
                    ...
                    if (buffer == "from")
                    {
                        ...
                        if (buffer == "right")
                        {
                            ...
                            if (autoPlayFromRight)
                            {
                                ...
                                std::cout << "		Scheduled playback from the rightmost continously reachable position from the beginning when the "
                                             "client starts ("
                                          << (buffer == "1") << ")." << std::endl;
                            }
                        }
                ...
                else if (buffer == "initial")
                {
                    ...
                    if (buffer == "bitfield")
                    {
                        ...
                        else if (buffer == "gaussian")
                        {
                            ...
                            {
                                NS_ABORT_MSG("[line " << currentLine
                                                      << "] Error: You must first define the mean (\"mean\") of the gaussian distribution.");
                            }
                            ...
                            {
                                NS_ABORT_MSG("[line " << currentLine
                                                      << "] Error: After the mean (\"mean\") you must define the standard deviation (\"stddev\") of "
                                                         "the gaussian distribution. (NOT the variance!)");
                            }
                            ...
                            for (NodeContainer::Iterator it = affectedNodes.Begin(); it != affectedNodes.End(); ++it)
                            {
                                ...
                                if (!randomTail)
                                {
                                    Simulator::Schedule(time,
                                                        &BitTorrentVideoClient::SetInitialBitfield,
                                                        dynamic_cast<BitTorrentVideoClient*>(PeekPointer((*it)->GetApplication(0))),
                                                        "p" + lexical_cast<std::string>(randomValue));
                                    std::cout << "		Node " << (*it)->GetId() << " has " << randomValue * 100
                                              << "% bitfield filling from the beginning." << std::endl;
                                }
                                else
                                {
                                    Simulator::Schedule(time,
                                                        &BitTorrentVideoClient::SetInitialBitfield,
                                                        dynamic_cast<BitTorrentVideoClient*>(PeekPointer((*it)->GetApplication(0))),
                                                        "q" + lexical_cast<std::string>(randomValue));
                                    std::cout << "		Node " << (*it)->GetId() << " has " << randomValue * 100
                                              << "% bitfield filling from the beginning, with a random tail for the following pieces." << std::endl;
                                }
                            }
                        }
                        else if (buffer == "from")
                        {
                            ...
                            {
                                NS_ABORT_MSG("[line " << currentLine
                                                      << "] Error: The filling percentage must be given as a value in the range [0.0, 1.0].");
                            }
                            ...
                            if (!randomTail)
                            {
                                SCHEDULE_CHAPTER(&BitTorrentVideoClient::SetInitialBitfield,
                                                 BitTorrentVideoClient,
                                                 "p" + lexical_cast<std::string>(percentage))
                            ...
                            else
                            {
                                SCHEDULE_CHAPTER(&BitTorrentVideoClient::SetInitialBitfield,
                                                 BitTorrentVideoClient,
                                                 "q" + lexical_cast<std::string>(percentage))
                                std::cout << "		Set initial bitfield to " << percentage * 100
                                          << "% filling from the beginning, with a random tail for the following pieces." << std::endl;
                            }
                        }
                        else if (buffer == "debug")
                        {
                            ...
                            std::cout << "		Set bitfield index " << bitfieldIndex << " to value " << value << " for debugging purposes."
                                      << std::endl;
                        }
                        else
                        {
                ...
                else if (buffer == "concurrent")
                {
                    ...
                    if (buffer == "requests")
                    {
                        ...
                        std::cout << "		Set number of maximum concurrent requests per peer (in pieces) to " << maxPeerRequests << "."
                                  << std::endl;
                    }
                    else
                    {
                ...
                else if (buffer == "video")
                {
                    ...
                    else if (buffer == "skip")
                    {
                        ...
                        if (buffer == "active")
                        {
                            ...
                            std::cout
                                << "		Set pieces missable during playback, subject to the further settings (\"tolerance\" and \"afterwards\")."
                                << std::endl;
                        }
                        ...
                        else if (buffer == "tolerance")
                        {
                            ...
                            std::cout << "		Set maximum tolerance for missing pieces to skip before onset of a buffering period to "
                                      << skipTolerance << " milliseconds." << std::endl;
                        }
                        else if (buffer == "afterwards")
                        {
                            ...
                            SCHEDULE_CHAPTER(&BitTorrentVideoClient::SetRequiredContinousPlaybackAfterSkip,
                                             BitTorrentVideoClient,
                                             MilliSeconds(skipTolerance))

                            std::cout << "		Set minimum length of required uninterrupted possible playback after a buffering period because of "
                                         "missing pieces to "
                                      << skipTolerance << " milliseconds." << std::endl;
                        }
                    }
            ...
            else if (buffer == "video")
            {
                ...
                else if (buffer == "skip")
                {
                    ...
                    if (buffer == "left")
                    {
                        ...
                        std::cout << "		Scheduled video fast forward to rightmost continously reachable position from the beginning."
                                  << std::endl;
                    }
                    else
                    {
                ...
                else if (buffer == "buffer")
                {
                    ...
                    else if (bufferingType == 1) // next
                    {
                        ...
                        std::cout << "            Scheduled video buffering until the next " << buffer << " ms in the video are available. "
                                  << std::endl;
                    }
                    else // until
                    {
                        ...
                        std::cout << "            Scheduled video buffering until the part at " << buffer << " ms in the video is available. "
                                  << std::endl;
                    }
                }
        ...
        else if (topology)
        {
            ...
            else if (buffer == "add")
            {
                ...
                if (buffer == "tapnode")
                {
                    ...
                    {
                        NS_ABORT_MSG("[line " << currentLine
                                              << "] Error: When adding a tapnode, please specify the name of the TapDevice as a first parameter.");
                    }
                    ...
                    if (buffer == "uplink")
                    {
                        ...
                        {
                            NS_ABORT_MSG("[line " << currentLine
                                                  << "] Error: When adding clients, please specify the downlink speed of the clients (\"downlink\") "
                    ...
                    m_tapNodes->Add(
                        m_topologyHelper->AddTapNodeToLastTopology(tapDevice, (*routerIt).second, uplinkSpeed, downlinkSpeed, "0ms", false, false));
                ...
                else if (buffer == "tracker")
                {
                    if (!m_trackerAdded)
                    {
                        ...
                        m_btTracker->Add(m_topologyHelper->AddNodeToLastTopology((*routerIt).second,
                                                                                 "point-to-point",
                                                                                 "100Mbps",
                                                                                 "100Mbps",
                                                                                 "0ms",
                ...
                else if (buffer == "clients")
                {
                    ...
                    if (buffer != "count")
                    {
                        NS_ABORT_MSG(
                            "[line "
                            << currentLine
                            << "] Error: When adding clients, please specify the number of the clients to add (\"count\") as a first parameter.");
                    }
                    ...
                    {
                        NS_ABORT_MSG("[line " << currentLine
                                              << "] Error: When adding clients, please specify the connection type of the clients to add (\"type\") "
                                                 "as a second parameter.");
                    }
                    ...
                    else if (buffer == "using")
                    {
                        ...
                        if (buffer != "samples")
                        {
                            NS_ABORT_MSG(
                                "[line "
                                << currentLine
                                << "] Error: When adding clients, you can either specify up- and downlink bandwidths or use the \"samples file\".");
                        }
                    ...
                    {
                        NS_ABORT_MSG("[line " << currentLine
                                              << "] Error: When adding clients not using the \"samples file\", please specify the channel delay of "
                                                 "the connection of the clients (\"delay\") as a third parameter.");
                    }
                    ...
                    if (!lineBuffer.eof())
                    {
                        ...
                        {
                            NS_ABORT_MSG("[line " << currentLine
                                                  << "] Error: When adding clients, you may specify to which router to add your nodes using the \"to "
                                                     "router\" argument.");
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        for (uint32_t i = 0; i < newNodeCount; ++i)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    NS_ABORT_MSG("[line " << currentLine
                                                          << "] Error: Could not find the router to attach the new clients to. Make sure that IDs "
                                                             "are sequential, starting at 0.");
                                }
                            }

                            newNodeContainer.Add(m_topologyHelper->AddNodeToLastTopology((*routerIt).second,
                                                                                         deviceType,
                                                                                         uplinkSpeed,
                                                                                         downlinkSpeed,
                                                                                         delay,
                ...
                else if (buffer == "other" || buffer == "background" || buffer == "sink")
                {
                    ...
                    if (buffer == "nodes")
                    {
                        ...
                        {
                            NS_ABORT_MSG("[line " << currentLine
                                                  << "] Error: When adding clients, please specify the connection type of the clients to add "
                        ...
                        if (buffer == "uplink")
                        {
                            ...
                            {
                                NS_ABORT_MSG("[line " << currentLine
                                                      << "] Error: When adding clients, please specify the downlink speed of the clients "
                                                         "(\"downlink\") as a second parameter.");
                            }
                        ...
                        else if (buffer == "using")
                        {
                            ...
                            {
                                NS_ABORT_MSG("[line " << currentLine
                                                      << "] Error: When adding clients, you can either specify up- and downlink bandwidths or use "
                                                         "the \"samples file\".");
                            }
                        ...
                        {
                            NS_ABORT_MSG("[line " << currentLine
                                                  << "] Error: When adding clients not using the \"samples file\", please specify the channel delay "
                                                     "of the connection of the clients (\"delay\") as a third parameter.");
                        }
                        ...
                        if (!lineBuffer.eof())
                        {
                            ...
                            {
                                NS_ABORT_MSG("[line " << currentLine
                                                      << "] Error: You may either specify an \"ip\" address for a default empty node or a "
                                                         "\"destination\" for a background traffic node.");
                            }
                        }
                        ...
                        if (!lineBuffer.eof())
                        {
                            ...
                            {
                                NS_ABORT_MSG("[line " << currentLine
                                                      << "] Error: When adding clients, you may specify to which router to add your nodes using the "
                                                         "\"to router\" argument.");
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            for (uint32_t i = 0; i < newNodeCount; ++i)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        NS_ABORT_MSG("[line " << currentLine
                                                              << "] Error: Could not find the router to attach the new nodes to. Make sure that IDs "
                                                                 "are sequential, starting at 0.");
                                    }
                                }
                        ...
                        if (background)
                        {
                            if (destination == "")
                            {
                                NS_ABORT_MSG(
                                    "[line "
                                    << currentLine
                                    << "} Error: When defining a \"background\" node, you must specify a \"destination\" ip for the traffic.");
                            }
                        ...
                        std::cout << "		Added " << newNodeCount << " " << (background ? "background" : (backgroundSink ? "sink" : "other"))
                                  << " nodes to the topology." << std::endl;
                    }
                    else
                    {
            ...
            if (buffer == "set")
            {
                ...
                else if (buffer == "pcap")
                {
                    ...
                    if (buffer == "enabled")
                    {
                        ...
                        {
                            NS_ABORT_MSG("[line " << currentLine
                                                  << "] Error: To enable PCAP tracing, specify the file name prefix (may include paths).");
                        }
                    }
                    else
                    {
