...
class VideoStreamServer : public Application
{
  public:
     ...
     * \param httpServer Pointer to this instance of VideoStreamServer, which is where
     *                   the trace originated.
     * \param socket Pointer to the socket where the connection is established.
     */
    typedef void (*ConnectionEstablishedCallback)(Ptr<const VideoStreamServer> httpServer,
                                                  Ptr<Socket> socket);
  ...
  private:
    ...
    uint32_t ServeFromTxBuffer(Ptr<Socket> socket, Ptr<Packet> packet);
    ...
    TracedCallback<Ptr<const VideoStreamServer>, Ptr<Socket>> m_connectionEstablishedTrace;
...
}; // end of `class VideoStreamServer`
