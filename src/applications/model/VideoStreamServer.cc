...
// static
TypeId
VideoStreamServer::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::VideoStreamServer")
            ...
            .AddAttribute("Mtu",
                          ...
                          TypeId::ATTR_GET,
                          UintegerValue(),
                          MakeUintegerAccessor(&VideoStreamServer::m_mtuSize),
            ...
            .AddTraceSource("MainObject",
                            ...
                            MakeTraceSourceAccessor(&VideoStreamServer::m_mainObjectTrace),
                            "ns3::HttpServer::HttpObjectCallback")
            .AddTraceSource("EmbeddedObject",
                            "An embedded object has been generated.",
                            MakeTraceSourceAccessor(&VideoStreamServer::m_embeddedObjectTrace),
                            "ns3::HttpServer::HttpObjectCallback")
            .AddTraceSource("Tx",
                            "A packet has been sent.",
                            MakeTraceSourceAccessor(&VideoStreamServer::m_txTrace),
                            "ns3::Packet::TracedCallback")
            .AddTraceSource("Rx",
                            "A packet has been received.",
                            MakeTraceSourceAccessor(&VideoStreamServer::m_rxTrace),
                            "ns3::Packet::PacketAddressTracedCallback")
            .AddTraceSource("RxDelay",
                            "A packet has been received with delay information.",
                            MakeTraceSourceAccessor(&VideoStreamServer::m_rxDelayTrace),
                            "ns3::Application::DelayAddressCallback")
            .AddTraceSource("StateTransition",
                            "Trace fired upon every HTTP client state transition.",
                            MakeTraceSourceAccessor(&VideoStreamServer::m_stateTransitionTrace),
                            "ns3::Application::StateTransitionCallback");
    return tid;
}

void
VideoStreamServer::SetMtuSize(uint32_t mtuSize)
{
...
Ptr<Socket>
VideoStreamServer::GetSocket() const
{
    return m_initialSocket;
}

VideoStreamServer::State_t
VideoStreamServer::GetState() const
{
    return m_state;
}

std::string
VideoStreamServer::GetStateString() const
{
...
// static
std::string
VideoStreamServer::GetStateString(VideoStreamServer::State_t state)
{
    switch (state)
    {
    case NOT_STARTED:
...
void
VideoStreamServer::StartApplication()
{
    NS_LOG_FUNCTION(this);
    ...
    if (m_state == NOT_STARTED)
    {
        ...
        m_initialSocket->SetAcceptCallback(
            MakeCallback(&VideoStreamServer::ConnectionRequestCallback, this),
            MakeCallback(&VideoStreamServer::NewConnectionCreatedCallback, this));
        m_initialSocket->SetCloseCallbacks(
            MakeCallback(&VideoStreamServer::NormalCloseCallback, this),
            MakeCallback(&VideoStreamServer::ErrorCloseCallback, this));
        m_initialSocket->SetRecvCallback(
            MakeCallback(&VideoStreamServer::ReceivedDataCallback, this));
        m_initialSocket->SetSendCallback(MakeCallback(&VideoStreamServer::SendCallback, this));
...
bool
VideoStreamServer::ConnectionRequestCallback(Ptr<Socket> socket, const Address& address)
{
...
void
VideoStreamServer::NewConnectionCreatedCallback(Ptr<Socket> socket, const Address& address)
{
    NS_LOG_FUNCTION(this << socket << address);

    socket->SetCloseCallbacks(MakeCallback(&VideoStreamServer::NormalCloseCallback, this),
                              MakeCallback(&VideoStreamServer::ErrorCloseCallback, this));
    socket->SetRecvCallback(MakeCallback(&VideoStreamServer::ReceivedDataCallback, this));
    socket->SetSendCallback(MakeCallback(&VideoStreamServer::SendCallback, this));
...
void
VideoStreamServer::NormalCloseCallback(Ptr<Socket> socket)
{
...
void
VideoStreamServer::ErrorCloseCallback(Ptr<Socket> socket)
{
    NS_LOG_FUNCTION(this << socket);
...
void
VideoStreamServer::ReceivedDataCallback(Ptr<Socket> socket)
{
    NS_LOG_FUNCTION(this << socket);
    ...
    while ((packet = socket->RecvFrom(from)))
    {
            ...
            if (InetSocketAddress::IsMatchingType(from))
            {
                ...
                m_txBuffer->RecordNextServe(socket,
                                        Simulator::Schedule(processingDelay,
                                                            &VideoStreamServer::ServeNewMainObject,
                                                            this,
        ...
        case ThreeGppHttpHeader::EMBEDDED_OBJECT:
            ...
            m_txBuffer->RecordNextServe(
                socket,
                Simulator::Schedule(processingDelay,
                                    &VideoStreamServer::ServeNewEmbeddedObject,
                                    this,
                                    socket),
                httpHeader.GetClientTs());
            break;
...
void
VideoStreamServer::SendCallback(Ptr<Socket> socket, uint32_t availableBufferSize)
{
...
void
VideoStreamServer::ServeNewMainObject(Ptr<Socket> socket, Address from, bool videoNak)
{
    NS_LOG_FUNCTION(this << socket);
    ...
    if(!m_txBuffer->IsBufferEmpty(socket))
    {
        Simulator::Schedule(MilliSeconds(20), &VideoStreamServer::ServeNewMainObject, this, socket, from, videoNak);
        return;
    }
...
void
VideoStreamServer::ServeNewEmbeddedObject(Ptr<Socket> socket)
{
    NS_LOG_FUNCTION(this << socket);
...
uint32_t
VideoStreamServer::ServeFromTxBuffer(Ptr<Socket> socket)
{
    NS_LOG_FUNCTION(this << socket);
...
uint32_t
VideoStreamServer::ServeFromTxBuffer(Ptr<Socket> socket, Ptr<Packet> packet)
{
    NS_LOG_FUNCTION(this << socket);
...
void
VideoStreamServer::SwitchToState(VideoStreamServer::State_t state)
{
    ...
    m_state = state;
    NS_LOG_INFO(this << " Server " << " VideoStreamServer " << oldState << " --> " << newState << ".");
