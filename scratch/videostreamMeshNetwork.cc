...
NS_LOG_COMPONENT_DEFINE("VideoStreamMeshNetwork");
...
main(int argc, char* argv[])
{
    ...
    LogComponentEnable("VideoStreamServer", LOG_LEVEL_ALL);
    LogComponentEnable("VideoStreamPCDN", LOG_LEVEL_ALL);
    LogComponentEnable("VideoStreamMeshNetwork", LOG_LEVEL_INFO);
...
#define GROUP_TMP_NODEID GROUP_NODE_NUM
    const int GROUP_NODE_NUM = ARG_GROUP_NODE_NUM;
    ...
    NodeContainer HigherNodes[CDN_NODE_NUM];
    ...
    for (int i = 0; i < CDN_NODE_NUM; i++)
    {
        HigherNodes[i].Create(PCDN_NDOE_NUM); // PCDN_NUM
        stack.Install(HigherNodes[i]);
    }
    ...
    PointToPointHelper higherP2PHelper[CDN_NODE_NUM][PCDN_NDOE_NUM + 1];
    Ipv4InterfaceContainer higherP2PInterface[CDN_NODE_NUM][PCDN_NDOE_NUM + 1];
    ...
    for (int i = 0; i < CDN_NODE_NUM; i++)
    {
        for (int j = 0; j < PCDN_NDOE_NUM; j++)
        {
            higherP2PHelper[i][j].SetDeviceAttribute("DataRate", StringValue("10Mbps"));
            higherP2PHelper[i][j].SetChannelAttribute("Delay", TimeValue(NanoSeconds(6560 * 2)));
        }
        // 这一条用于连接PCDN的router和CDN节点
        higherP2PHelper[i][PCDN_NDOE_NUM].SetDeviceAttribute("DataRate", StringValue("100Mbps"));
        higherP2PHelper[i][PCDN_NDOE_NUM].SetChannelAttribute("Delay",
                                                              TimeValue(NanoSeconds(6560)));
    }
    ...
    for (int i = 0; i < CDN_NODE_NUM; i++)
    {
        for (int j = 0; j < PCDN_NDOE_NUM; j++)
        {
            device = higherP2PHelper[i][j].Install(HigherNodes[i].Get(j),
                                                   CDNNodes.Get(CDN_TMP_NODEID(i)));
            higherP2PInterface[i][j] = address.Assign(device);
            NS_LOG_INFO("PCDN to router_CDN : " << higherP2PInterface[i][j].GetAddress(0)
                                                << "------"
                                                << higherP2PInterface[i][j].GetAddress(1));

            address.NewNetwork();
        }
        // 这一条用于连接PCDN的router和CDN节点
        device = higherP2PHelper[i][PCDN_NDOE_NUM].Install(CDNNodes.Get(CDN_SERVER_NODEID(i)),
                                                           CDNNodes.Get(CDN_TMP_NODEID(i)));
        higherP2PInterface[i][PCDN_NDOE_NUM] =
            address.Assign(device); // get 0.address == CDN server address
        NS_LOG_INFO("CDN to router_CDN : " << higherP2PInterface[i][PCDN_NDOE_NUM].GetAddress(0)
                                           << "------"
                                           << higherP2PInterface[i][PCDN_NDOE_NUM].GetAddress(1));
    ...
    for (int i = 0; i < CDN_NODE_NUM; i++)
    {
        VideoStreamServerHelper serverHelper(higherP2PInterface[i][PCDN_NDOE_NUM].GetAddress(0));
    ...
    int videoRequestNumber[CDN_NODE_NUM][PCDN_NDOE_NUM][GROUP_NODE_NUM] = {};
    for(int i = 0; i < CDN_NODE_NUM; i++)
    {
        for(int j = 0; j < PCDN_NDOE_NUM; j++)
        {
            for(int k = 0; k < GROUP_NODE_NUM; k++)
            {
                videoRequestNumber[i][j][k] = distribution(gen);
            }
        }
    };
    for(int i = 0; i < CDN_NODE_NUM; i++)
    {
        for(int j = 0; j < PCDN_NDOE_NUM; j++)
        {
            for(int k = 0; k < GROUP_NODE_NUM; k++)
            {
                NS_LOG_INFO("videoRequestNumber[" << i << "][" << j << "][" << k << "] = " << videoRequestNumber[i][j][k]);
            }
        }
    };
        ...
        for (int i = 0; i < PCDN_NDOE_NUM; i++)
        {
            for (int j = 0; j < GROUP_NODE_NUM; j++)
            {
                VideoStreamClientHelper clientHelper(
                    clientP2PInterface[k][i][GROUP_TMP_NODEID].GetAddress(0));
                ApplicationContainer clientApps = clientHelper.Install(clientNodes[k][i].Get(j));
                clientApps.Stop(Seconds(simTimeSec));
                Ptr<VideoStreamClient> httpClient =
                    clientApps.Get(0)->GetObject<VideoStreamClient>();
                httpClient->SetVideoInfos(videoRequestNumber[k][i][j]);
                clientApps.Start(Seconds(j));
            }
        }
    }
    ...
    for (int k = 0; k < CDN_NODE_NUM; ++k)
    {
        for (int i = 0; i < PCDN_NDOE_NUM; i++)
        {
            ...
            anim.SetConstantPosition(clientNodes[k][i].Get(GROUP_TMP_NODEID),
                                     (clientRouterPos) * 4,
                                     2 * 4);
            anim.SetConstantPosition(HigherNodes[k].Get(i), (clientRouterPos++) * 4, 3 * 4);
        }
    }
    for (int i = 0; i < CDN_NODE_NUM; i++)
    {
    ...
    fileHelper.ConfigureFile("MytestCountsMesh", FileAggregator::TAB_SEPARATED);
    ...
    timeCounter.WriteProbe("ns3::TimeProbe", "/NodeList/*/ApplicationList/0/$ns3::VideoStreamClient/FirstTime", "Output");
